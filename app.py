...
<style>
    ...
    /* AI 分析区专属样式：字体更小、更紧凑 */
    .rebuttal-container { 
        background: #fff; border: 1px solid #E1BEE7; padding: 12px; border-radius: 12px; 
...
def chat_with_ai(prompt_type, content, api_key, api_base):
    ...
    except Exception as e: return f"AI 休息中..."
...
with header_left:
    ...
    new_idea = st.text_area(
        "灵感入口...", height=100, key="idea_in", label_visibility="collapsed",
        placeholder="『示例』：我想学滑板、去宿舍楼下卖旧书..."
    )
...
with header_right:
    if st.session_state.heavy_advice:
        ...
        st.markdown("<div class='ai-label'>🔍 AI 分析 (重力感应介入)</div>", unsafe_allow_html=True)
        # 字体缩小的建议区
        ...
        st.markdown("<div class='guide-box' style='height:215px; display:flex; align-items:center; justify-content:center;'>⚖️ AI 分析区 (静默中)</div>", unsafe_allow_html=True)
